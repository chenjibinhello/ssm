package com.learning.demo.bean.domain.erp;

import java.io.Serializable;

public class ErpInventoryBillProperties implements Serializable {
    private Integer propertyId;

    private Integer erpId;

    private String name;

    private String code;

    private String isAutocoding;

    private String usedNumber;

    private Boolean stockImpact;

    private Byte changeType;

    private String costImpact;

    private Boolean moneyPrecision;

    private String updateStorageIn;

    private String updateStorageOut;

    private String updateCheck;

    private Integer accountingSubject;

    private String directConfirm;

    private String directPrint;

    private Integer created;

    private Integer updated;

    private Byte isDeleted;

    private static final long serialVersionUID = 1L;

    public Integer getPropertyId() {
        return propertyId;
    }

    public void setPropertyId(Integer propertyId) {
        this.propertyId = propertyId;
    }

    public Integer getErpId() {
        return erpId;
    }

    public void setErpId(Integer erpId) {
        this.erpId = erpId;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name == null ? null : name.trim();
    }

    public String getCode() {
        return code;
    }

    public void setCode(String code) {
        this.code = code == null ? null : code.trim();
    }

    public String getIsAutocoding() {
        return isAutocoding;
    }

    public void setIsAutocoding(String isAutocoding) {
        this.isAutocoding = isAutocoding == null ? null : isAutocoding.trim();
    }

    public String getUsedNumber() {
        return usedNumber;
    }

    public void setUsedNumber(String usedNumber) {
        this.usedNumber = usedNumber == null ? null : usedNumber.trim();
    }

    public Boolean getStockImpact() {
        return stockImpact;
    }

    public void setStockImpact(Boolean stockImpact) {
        this.stockImpact = stockImpact;
    }

    public Byte getChangeType() {
        return changeType;
    }

    public void setChangeType(Byte changeType) {
        this.changeType = changeType;
    }

    public String getCostImpact() {
        return costImpact;
    }

    public void setCostImpact(String costImpact) {
        this.costImpact = costImpact == null ? null : costImpact.trim();
    }

    public Boolean getMoneyPrecision() {
        return moneyPrecision;
    }

    public void setMoneyPrecision(Boolean moneyPrecision) {
        this.moneyPrecision = moneyPrecision;
    }

    public String getUpdateStorageIn() {
        return updateStorageIn;
    }

    public void setUpdateStorageIn(String updateStorageIn) {
        this.updateStorageIn = updateStorageIn == null ? null : updateStorageIn.trim();
    }

    public String getUpdateStorageOut() {
        return updateStorageOut;
    }

    public void setUpdateStorageOut(String updateStorageOut) {
        this.updateStorageOut = updateStorageOut == null ? null : updateStorageOut.trim();
    }

    public String getUpdateCheck() {
        return updateCheck;
    }

    public void setUpdateCheck(String updateCheck) {
        this.updateCheck = updateCheck == null ? null : updateCheck.trim();
    }

    public Integer getAccountingSubject() {
        return accountingSubject;
    }

    public void setAccountingSubject(Integer accountingSubject) {
        this.accountingSubject = accountingSubject;
    }

    public String getDirectConfirm() {
        return directConfirm;
    }

    public void setDirectConfirm(String directConfirm) {
        this.directConfirm = directConfirm == null ? null : directConfirm.trim();
    }

    public String getDirectPrint() {
        return directPrint;
    }

    public void setDirectPrint(String directPrint) {
        this.directPrint = directPrint == null ? null : directPrint.trim();
    }

    public Integer getCreated() {
        return created;
    }

    public void setCreated(Integer created) {
        this.created = created;
    }

    public Integer getUpdated() {
        return updated;
    }

    public void setUpdated(Integer updated) {
        this.updated = updated;
    }

    public Byte getIsDeleted() {
        return isDeleted;
    }

    public void setIsDeleted(Byte isDeleted) {
        this.isDeleted = isDeleted;
    }

    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder();
        sb.append(getClass().getSimpleName());
        sb.append(" [");
        sb.append("Hash = ").append(hashCode());
        sb.append(", propertyId=").append(propertyId);
        sb.append(", erpId=").append(erpId);
        sb.append(", name=").append(name);
        sb.append(", code=").append(code);
        sb.append(", isAutocoding=").append(isAutocoding);
        sb.append(", usedNumber=").append(usedNumber);
        sb.append(", stockImpact=").append(stockImpact);
        sb.append(", changeType=").append(changeType);
        sb.append(", costImpact=").append(costImpact);
        sb.append(", moneyPrecision=").append(moneyPrecision);
        sb.append(", updateStorageIn=").append(updateStorageIn);
        sb.append(", updateStorageOut=").append(updateStorageOut);
        sb.append(", updateCheck=").append(updateCheck);
        sb.append(", accountingSubject=").append(accountingSubject);
        sb.append(", directConfirm=").append(directConfirm);
        sb.append(", directPrint=").append(directPrint);
        sb.append(", created=").append(created);
        sb.append(", updated=").append(updated);
        sb.append(", isDeleted=").append(isDeleted);
        sb.append(", serialVersionUID=").append(serialVersionUID);
        sb.append("]");
        return sb.toString();
    }
}